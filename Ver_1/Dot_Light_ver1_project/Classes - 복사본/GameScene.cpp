...
void GameScene::update_line(float dt){
	rend1->go_raycast_angle(player->GetPosition(),r_angle,world);
	light->setPosition(rend1->get_resultp());
	rend2->go_raycast_angle(rend1->get_result(),rend1->get_ref_angle(),world);
...
void GameScene::draw(Renderer* renderer, const kmMat4& transform, uint32_t transformUpdated)
{
	...
	if(isbg)ccDrawSolidRect(Vec2(0,0),winSize,Color4F(0,1,0,255));
	glLineWidth(2);
